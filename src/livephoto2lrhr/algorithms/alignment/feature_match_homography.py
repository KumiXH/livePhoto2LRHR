...
class FeatureMatchHomographyAligner:
    ...
    def align(self, lr_rgb: np.ndarray, hr_rgb: np.ndarray, context: AlignmentContext) -> AlignResult:
        try:
            ...
            if post_error >= pre_error:
                return self._failed_result(
                    "feature matching homography did not improve alignment error",
                    diagnostics={
                        "algorithm": "feature_match_homography",
                        "detector": self.detector,
                        "keypoints_lr": len(lr_keypoints),
            ...
            return self._failed_result(
                str(exc),
                diagnostics={"algorithm": "feature_match_homography", "detector": self.detector},
            )
